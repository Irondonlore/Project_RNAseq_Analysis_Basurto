...
info.file <- "X:/irondon/Project_AC-45_RNAseq-FFPE/RNAseq/4_STEP_DATA_ANALYSIS/4_Data_Basurto/Results/Sample_info_table/sample_info_signatures_EMT_WNT.txt"
counts.file <- "X:/irondon/Project_AC-45_RNAseq-FFPE/RNAseq/4_STEP_DATA_ANALYSIS/4_Data_Basurto/Results/VST_Counts/FullCounts_Basurto_cpm_filtered_phenotype_VST_PTEN_loss_intact.txt"
...
#| Filtering BPH
#|counts_data <- counts_data[,datTraits$AC.basurto[which(datTraits$Diagnostico == "PCa")]]
...
datTraits <- subset( datTraits, select = c( Edad, 
                                            ...
                                            Mean.expression.ETM.pathway,
                                            Mean.expression.WNT.pathway,
                                            H_score_cut_0,
                                            H_score_cut_10,
                                            H_score_cut_20,
                                            H_score_cut_30,
...
if (dim(counts_data)[2] == dim(datTraits)[1]){
  ...
  sizeGrWindow(12,9)
  pdf(file = "Results/Images/0_sampleClustering.pdf")
  par(cex = 0.6)
  par(mar = c(0,4,2,0))
...
if (dim(counts_data)[2] == dim(datTraits)[1]){
  ...
  pdf("Images/0_sample_dendrogram_and_trait_heatmap.pdf")
  plotDendroAndColors(sampleTree, traitColors,
                      groupLabels = names(datTraits),
                      main = "Sample dendrogram and trait heatmap", cex.colorLabels = 0.5, cex.dendroLabels = 0.2, family ="serif")
  dev.off()
  
  save(datExpr, datTraits, file = paste("Results/Data/", tag, "_dataInput.RData", sep =""))
