...
wgcna_Modules <- read.table("X:/irondon/Project_AC-45_RNAseq-FFPE/RNAseq/5_STEP_WGCNA/5_WGCNA_PCa/Results/Tables/3_geneInfo_counts_geneMM_geneS_moduleColors.txt",  sep = "\t")
...
module_eigengenes <- module_eigengenes[which(!is.na(datTraits$DFS.STATUS)),]
datTraits

#| Create the design matrix
des_mat <- model.matrix(~datTraits$DV200.value +datTraits$Edad + datTraits$H_score_cut_0 )
module_eigengenes <- module_eigengenes[which(!is.na(datTraits$H_score_cut_0)),]

#| Run linear model on each module. Limma wants our tests to be per row, so we also 
#| need to transpose so the eigengenes are rows lmFit() needs a transposed version 
#| of the matrix
...
#| Removing intercept from test coefficients. Let's take a look at the results. They 
#| are sorted with the most significant results at the top.
head(stats_df)
