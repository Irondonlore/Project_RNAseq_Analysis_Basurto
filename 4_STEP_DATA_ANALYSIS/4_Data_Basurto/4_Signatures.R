...
etm.file <- "X:/irondon/Project_AC-45_RNAseq-FFPE/RNAseq/4_STEP_DATA_ANALYSIS/4_Data_Basurto/Data/EMT-genes.txt"
wnt.file <- "X:/irondon/Project_AC-45_RNAseq-FFPE/RNAseq/4_STEP_DATA_ANALYSIS/4_Data_Basurto/Data/WNT-genes.txt"
...
mir_19a_3p.file <- "X:/irondon/Project_AC-45_RNAseq-FFPE/RNAseq/5_STEP_WGCNA/5_WGCNA_PCa/MIR_19a-3p_gene_name.txt"
mir_106a_5p.file <- "X:/irondon/Project_AC-45_RNAseq-FFPE/RNAseq/5_STEP_WGCNA/5_WGCNA_PCa/MIR-106a-5p_genes_name.txt"
setwd(dir.proj)
...
#| ETM
ETM_signature <-  read.table(etm.file, sep =",", quote="",header=T)

#| WNT
WNT_signature <- read.table(wnt.file, sep =",", quote="",header=T)

#| mir-19a-3p
mir_19a_3p <- read.table(mir_19a_3p.file, sep ="\t", header=T)

#| mir-106a-5p
mir_106a_5p <- read.table(mir_106a_5p.file, sep ="\t", header=T)
################################################################################
...
############################  MIR-19a-3p SIGNATURE  ############################
mir_19a_3p <- mir_19a_3p$Target

#| Finding gene of the FOXO signature inside the gene expression data
counts_mir_19a_3p <- counts_zscore[which(rownames(counts_zscore) %in% genome_GRCh39.94$GeneID[which(genome_GRCh39.94$gene_name %in% mir_19a_3p)]),]

#| Computing the mean over all the columns (all samples)
samples_media_expression <- colMeans(counts_mir_19a_3p, na.rm=TRUE)

#| Saving to the sample_info table
sample_info$mir_19a_3p <- samples_media_expression

ggplot(sample_info[which(!is.na(sample_info$H_score_cut_0)),], aes(x=H_score_cut_0, y = mir_19a_3p, fill =H_score_cut_0)) +
  geom_boxplot() +
...
###########################  MIR-106a-5p SIGNATURE  ############################
mir_106a_5p <- mir_106a_5p$Target

#| Finding gene of the FOXO signature inside the gene expression data
counts_mir_106a_5p <- counts_zscore[which(rownames(counts_zscore) %in% genome_GRCh39.94$GeneID[which(genome_GRCh39.94$gene_name %in% mir_106a_5p)]),]

#| Computing the mean over all the columns (all samples)
samples_media_expression <- colMeans(counts_mir_106a_5p, na.rm=TRUE)

#| Saving to the sample_info table
sample_info$mir_106a_5p <- samples_media_expression

ggplot(sample_info[which(!is.na(sample_info$H_score_cut_0)),], aes(x=H_score_cut_0, y = mir_106a_5p, fill =H_score_cut_0)) +
  geom_boxplot() +
  theme(text=element_text(size=16,  family="serif"), axis.text=element_text(size=12), plot.title=element_text(size=16, hjust = 0.5, face ="bold"))+
  scale_fill_manual(values = c("darkgoldenrod3", "midnightblue")) + 
  stat_compare_means(label ="p.format",family="serif", size =5)
...
#| Saving to the sample_info table
sample_info$`Mean expression PI3K-AKT-mTOR` <- samples_media_expression

names(sample_info)
sample_info$`Mean expression PI3K-AKT-mTOR`
...
ggplot(sample_info, aes(x =Diagnostico, y =`Mean expression PI3K-AKT-mTOR`, fill =Diagnostico)) +
  geom_boxplot()+ theme(text=element_text(size=16,  family="serif"), axis.text=element_text(size=12), plot.title=element_text(size=16, hjust = 0.5, face ="bold")) + 
...
#############################  ETM PATHWAY  ####################################
#| Converting it to data.frame
ETM_signature <- data.frame(gene_name = colnames(ETM_signature))

#| Merging gene names to the foxo signature
ETM_signature <- merge(ETM_signature,genome_GRCh39.94, by ="gene_name" )  #dim = 842 10

#| Finding gene of the pi3k_akt_mtor signature inside the gene expression data
counts_etm_pathway <- counts_zscore[which(rownames(counts_zscore) %in% ETM_signature$GeneID),]

#| Computing the mean over all the columns (all samples)
samples_media_expression <- colMeans(counts_etm_pathway, na.rm=TRUE)

#| Saving to the sample_info table
sample_info$`Mean expression ETM pathway` <- samples_media_expression
################################################################################
...
#############################  WNT PATHWAY  ####################################
#| Converting it to data.frame
WNT_signature <- data.frame(gene_name = colnames(WNT_signature))

#| Merging gene names to the foxo signature
WNT_signature <- merge(WNT_signature,genome_GRCh39.94, by ="gene_name" )  #dim = 842 10

#| Finding gene of the pi3k_akt_mtor signature inside the gene expression data
counts_wnt_pathway <- counts_zscore[which(rownames(counts_zscore) %in% WNT_signature$GeneID),]

#| Computing the mean over all the columns (all samples)
samples_media_expression <- colMeans(counts_wnt_pathway, na.rm=TRUE)

#| Saving to the sample_info table
sample_info$`Mean expression WNT pathway` <- samples_media_expression
################################################################################


write.table(sample_info, "Results/Sample_info_table/sample_info_signatures_EMT_WNT.txt", sep = "\t")
