...
################################################################################
############### TO CHECK IF ALL THE FILES WERE SUCCESSFULLY RUN ################
#Samples_completed <- read.table("X:/irondon/Project_AC-45_RNAseq-FFPE/RNAseq/FASTQC_completed.txt", sep = "\t")
#Samples_completed <- Samples_completed$V1
#Samples_completed <- substring(Samples_completed, first = 23, last = 33)
#Samples_completed <- gsub(".[a-z]*$", "", Samples_completed)
#Samples_completed <- sort(Samples_completed)
#Checking if they contain the same samples
#matches <- Samples %in% Samples_completed
#The ones that does not match:
#Samples[matches == FALSE]   # AC16_2 and AC54_2
################################################################################
